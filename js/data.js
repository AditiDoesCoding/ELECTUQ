...
export const journeySteps = [
    ...
    { id: 's2', title: 'Step 2: Form 6 Submission', desc: 'New voter? Submit Form 6 for fresh registration or address change.', interactive: '<a href="https://voters.eci.gov.in/" target="_blank" class="btn btn-primary" style="display: inline-block; text-decoration: none; padding: 8px 15px; font-size: 0.85rem;" onclick="event.stopPropagation();">Open Voter Portal ↗</a>' },
    { id: 's3', title: 'Step 3: e-EPIC Download', desc: 'Get your digital Voter ID. Essential for fast-track voting.', interactive: `
        <div style="display:flex; flex-direction:column; gap:8px;">
            <input type="text" placeholder="Enter EPIC Number" style="padding: 8px; border-radius: 8px; border: 1px solid var(--border);" onclick="event.stopPropagation();">
            <button class="btn btn-white" style="border: 1px solid var(--border); font-size: 0.75rem;" onclick="alert('Google Cloud Vision AI: Scanning simulation active... (Verification would happen via OCR in production)'); event.stopPropagation();">
                📸 Scan ID via Vision AI
            </button>
        </div>` },
    ...
    { id: 's6', title: 'Step 6: Candidate Research', desc: 'Read criminal antecedents and assets of local candidates.', interactive: '<a href="https://myneta.info/" target="_blank" class="btn btn-white" style="display: inline-block; text-decoration: none; border: 1px solid var(--border); padding: 8px 15px; font-size: 0.85rem; font-weight: 700; color: inherit;" onclick="event.stopPropagation();">Research Candidates 🕵️</a>' },
    { id: 's7', title: 'Step 7: Mock Vote Practice', desc: 'Practice the EVM+VVPAT sequence to avoid errors.', interactive: '<button class="btn btn-white" style="border: 1px solid var(--border); padding: 8px 15px; font-size: 0.85rem;" onclick="window.showTab(\'home\'); document.getElementById(\'home-booth-sim\').scrollIntoView({behavior:\'smooth\'}); event.stopPropagation();">Launch Simulator ✨🐘😊</button>' },
